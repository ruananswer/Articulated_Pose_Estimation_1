function boxes = detect_fast(im, model, thresh, modelR, modelNeg)
...
for rlevel = levels,
  for c  = 1:length(model.components),
    ...
    % predict
    %%{
    pyra_feat = reshape(pyra.featR{rlevel}, [size(pyra.featR{rlevel}, 1)*size(pyra.featR{rlevel}, 2) model.featureDimen]);
    ABSMAX = max(pyra_feat) + eps;
    pyraX = sparse(bsxfun(@times, pyra_feat, 1./ABSMAX));
    [lab_neg, acc_neg, votes_neg] = predict(zeros(size(pyra.featR{rlevel}, 1)*size(pyra.featR{rlevel},2),1), pyraX, modelNeg);
    Y_hat_neg = reshape(votes_neg, [size(pyra.featR{rlevel}, 1) size(pyra.featR{rlevel}, 2)]);
    Y_hat_neg = max(Y_hat_neg + 1,0);
    [lab, acc, votes] = predict(zeros(size(pyra.featR{rlevel}, 1)*size(pyra.featR{rlevel},2),1), pyraX, modelR);
    labb = reshape(lab, [size(pyra.featR{rlevel}, 1) size(pyra.featR{rlevel}, 2)]);
    for k = 1 : numparts
        Y_hat_temp{k} = reshape(votes(:,k), [size(pyra.featR{rlevel}, 1) size(pyra.featR{rlevel}, 2)]);
        temp = (labb == k);
        %Y_hat{k} = temp .* Y_hat_temp{k};   
        %Y_hat{k} = max(Y_hat{k},0);
        Y_hat{k} = temp;
    end
    %}
    % Local scores
    for k = 1:numparts,
      ...
      if isempty(resp{level}),
        ...
        for ii = 1 : size(filters,1)
            resp{level}{ii} = resp{level}{ii} .* Y_hat{k}; 
        end
      end
      for fi = 1:length(f)
        parts(k).score(:,:,fi) = resp{level}{f(fi)};
       % parts(k).score(:,:,fi) = Y_hat{k};
      end
      parts(k).level = level;
    end
    ...
    if length(X) > 1,
      ...
      boxes(i,:) = [box repmat(c,length(I),1) reshape(rscore(I),length(I), 1)];
      cnt = i(end);
    end
  end
end
...
function box = backtrack(x,y,mix,parts,pyra)
  ...
  for k = 1:numparts,
    ...
    x2 = x1 + p.sizx(mptr(:,k))*scale*5 - 1;
    y2 = y1 + p.sizy(mptr(:,k))*scale*5 - 1;    
    %x1 = (xptr(:,k) + 2 - pyra.padx)*scale+1;
    %y1 = (yptr(:,k) + 2 - pyra.pady)*scale+1;
    %x2 = x1 + p.sizx(mptr(:,k))*scale*5 - 1;
    %y2 = y1 + p.sizy(mptr(:,k))*scale*5 - 1;
    % for 1X1
    %x1 = xptr(:, k)*scale + 1;
