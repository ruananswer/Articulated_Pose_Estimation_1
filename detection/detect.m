...
x2  = x1 + p.sizx(mix)*scale*5 - 1;
y2  = y1 + p.sizy(mix)*scale*5 - 1;
...
for k = 2:numparts
	...
	x2  = x1 + p.sizx(ptr(k,3))*scale*5 - 1;
	y2  = y1 + p.sizy(ptr(k,3))*scale*5 - 1;
...
x2 = x1 + sizx*scale*5 - 1;
y2 = y1 + sizy*scale*5 - 1;
%}

%x1 = ((1:dimx-sizx+1) - padx - 1)*scale*2 + 1;
%y1 = ((1:dimy-sizy+1) - pady - 1)*scale*2 + 1;
%x2 = x1 + sizx*scale*7 - 1;
%y2 = y1 + sizy*scale*7 - 1;

%x1  = ((1:dimx-sizx+1) - padx + 7)*scale+1;
%y1  = ((1:dimy-sizy+1) - pady + 8)*scale+1;
%x2  = x1 + sizx*scale*5 - 1;
%y2  = y1 + sizy*scale*5 - 1;
